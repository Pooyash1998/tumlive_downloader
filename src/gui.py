...
import tum_live
import downloader
from main import load_config_file, parse_destination_folder, parse_tmp_folder, parse_maximum_parallel_downloads, parse_username_password
...
class TUMLiveDownloaderGUI:
    def __init__(self, root):
        ...
        self.username, self.password = parse_username_password(self.config) if self.config else (None, None)
    ...
    def load_config(self):
        """Load configuration using main.py functions"""
        try:
    ...
    def show_main_page(self):
        ...
        # Get config values using main.py functions
        try:
            output_dir = parse_destination_folder(self.config)
            tmp_dir = parse_tmp_folder(self.config)
            max_downloads = parse_maximum_parallel_downloads(self.config).get_value()
        except:
            output_dir = Path.home() / "Downloads"
            tmp_dir = Path.home() / "tmp" / "tum_video_scraper"
            max_downloads = 3
        ...
        self.max_parallel_downloads = max_downloads
        ...
        try:
            ...
            for i, (subject, playlists) in enumerate(videos_for_subject.items()):
                ...
                spawned_processes += downloader.download_list_of_videos(
                    ...
                    threading.Semaphore(self.max_parallel_downloads)
                )
